...
exports.run = async(client, message, args) => {
    ...
    .setColor('BLACK')
    .setDescription(`Whitelist vermek için <@&${ayarlar.wlyetkili}> rolüne sahip olman gerekiyor!`)
   ...
   if(!message.member.roles.cache.has(ayarlar.wlyetkili)) {
    ...
    .setAuthor(message.author.username, message.author.avatarURL())
    .setColor('BLUE')
    .setDescription(`${user} üyesi, ${message.author} tarafından **whitelist** verildi.`)


    const darkbiat34 = new Discord.MessageEmbed()
    .setAuthor(message.author.username, message.author.avatarURL())
    .setColor('BLUE')
    .setDescription(`Whitelist verilicek kişi belirtilmedi.`)
    ...
    ${user} (${args[0]}) kişisine whitelisted eklendi!
...
user.roles.add(ayarlar.wlrol1)
user.roles.add(ayarlar.wlrol2)
